...
const CreatePageUseGuide = ({ toggleGuide, setGuide }) => {
  ...
  const control = (
    <div className={classes.control}>
...

      {
        (pointer < guideLength)
          ? (
            <button type="button" onClick={handleNext} className={`${classes.next} ${pointer < guideLength && classes.active}`}>
              next
              {' '}
          ...
          : (
            <button type="button" onClick={handleCancel} className={`${classes.next} ${pointer < guideLength && classes.active}`}>
              close
              {' '}
  ...
  const introControl = (
    <div className={classes.introControl}>
      ...
      <button type="button" onClick={() => handleSetState({ showGuide: true })}>Get started</button>
    </div>
  );
