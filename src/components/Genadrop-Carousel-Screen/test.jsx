...
const GenadropCarouselScreen = ({ children, cardWidth, gap = 16 }) => {
  ...
  const handleSlideLeft = () => {
    ...
    if (slideActiveCount > slideCount) return setSlideActiveCount(slideCount - 2);
    setSlideActiveCount((sc) => sc - 1);
  };

  const handleSlideRight = () => {
    console.log({ slideActiveCount });
    if (slideActiveCount === slideCount) return;
    if (slideActiveCount > slideCount) return setSlideActiveCount(slideActiveCount - slideCount);
    setSlideActiveCount((sc) => sc + 1);
  };
      ...
      <div ref={wrapperRef} className={classes.wrapper}>
        ...
        {slideCount ? (
          <>
            ...
            <button onClick={handleSlideRight} className={`${classes.ctrlBtn_right} ${true && classes.active}`}>
