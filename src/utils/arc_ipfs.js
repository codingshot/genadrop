...
export async function mintSingleToPoly(singleMintProps) {
  const { file, metadata, price, account, connector, dispatch, setLoader, mainnet } =
    singleMintProps;
...
export async function mintSingleToCelo(singleMintProps) {
  const { file, metadata, account, connector, dispatch, setLoader, mainnet } =
    singleMintProps;
  if (connector.isWalletConnect) {
    if (connector.chainId === 137) {
      return { message: "not yet implemented" };
...
export async function mintToAlgo(algoProps) {
  ...
  dispatch(
    setNotification("connect wallet to algorand network or select a different chain")
  );
}

export async function mintToCelo(celoProps) {
  const { account, connector, fileName, dispatch, setNotification, setLoader, mainnet } =
    celoProps;
