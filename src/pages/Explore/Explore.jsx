...
const Explore = () => {
  ...
  useEffect(() => {
    ;(async function getGraphResult() {
      ...
      const filteredCollection = allCollection?.filter(
        (col) => col?.Id === collectionName,
      )
      if (filteredCollection?.length) {
        const result = await getGraphCollection(
          filteredCollection[0]?.nfts,
        ...
        <div className={classes.header}>
          <div
            className={`${classes.tab} && ${
              seleted == 'items' && classes.tabActive
            }`}
          ...
          <div
            className={`${classes.tab} && ${
              seleted == 'activity' && classes.tabActive
            }`}
            onClick={() =>
              handleSetState({ seleted: 'activity', firstClick: true })
            }
          >
