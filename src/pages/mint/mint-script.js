...
export async function mintToCelo(polyProps) {
  const { window, ipfsJsonData, account, connector, mintFileName, dispatch, setNotification, setLoader } = polyProps;
...
export const handleMint = async props => {
  ...
  try {
    if (selectValue.toLowerCase() === 'algo') {
      url = await mintToAlgo({ window, ipfsJsonData, account, connector, mintFileName, dispatch, setNotification, setLoader, priceValue });
